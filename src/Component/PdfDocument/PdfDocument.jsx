...
const App = () => {
  const exportPdfHandler = () => {
    ...
    for (let page = 1; page <= pageCount; page++) {
      ...
      // Add header image
      doc.addImage(headerImage, "PNG", 20, 12, 15, 10); // Adjust position and size as needed

      // Set header text (Hamza) in the top-right corner
      doc.setFontSize(12);
      doc.text("Hamza", doc.internal.pageSize.width - 30, 20);
      ...
      const endIndex = Math.min(page * 30, data.length);
      const pageData = data.slice(startIndex, endIndex);

      // Add separator line in the header
      doc.setLineWidth(0.5); // Set the width of the line
      doc.setDrawColor(0); // Set the color of the line (black)
      doc.line(10, 25, doc.internal.pageSize.width - 10, 25); // Add a horizontal line in the header

      // Add separator line in the footer
      doc.setLineWidth(0.5); // Set the width of the line
      doc.setDrawColor(0); // Set the color of the line (black)
      doc.line(
        10,
        ...
        doc.internal.pageSize.height - 15
      ); // Add a horizontal line in the footer
      doc.autoTable({
        head: [["ID", "Description", "Quantity", "Price"]],
        ...
        startY: 30, // Adjust the start position of the table
        theme: "plain", // Change the theme to 'grid'
        styles: {
          fontSize: 10, // Change the font size to 10
          valign: "middle",
          halign: "center",
        },
        margin: { top: 40 }, // Add margin to accommodate the header
        repeatHeader: true, // Repeat the header on each page
      });
    }
  ...
  return (
    <div>
      <button style={Butto} onClick={exportPdfHandler}>
        Export PDF
      </button>
    </div>
  );
